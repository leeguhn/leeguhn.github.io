(() => {
  ...
  const state = { mode: "baseline", paused: false, editing: true, selected: "A", voiceLow: defaults.voiceLow, voiceHigh: defaults.voiceHigh, hrtfLow: defaults.hrtfLow, hrtfHigh: defaults.hrtfHigh, listener: { ...defaults.listener }, obstacles: structuredClone(defaults.obstacles), startedAt: performance.now() };
  ...
  function drawArrow(x1, y1, x2, y2) { const angle = Math.atan2(y2 - y1, x2 - x1), size = 12; radarCtx.beginPath(); radarCtx.moveTo(x2, y2); radarCtx.lineTo(x2 - Math.cos(angle - 0.5) * size, y2 - Math.sin(angle - 0.5) * size); radarCtx.lineTo(x2 - Math.cos(angle + 0.5) * size, y2 - Math.sin(angle + 0.5) * size); radarCtx.closePath(); radarCtx.fill(); radarCtx.beginPath(); radarCtx.moveTo(x1, y1); radarCtx.lineTo(x1 + Math.cos(angle - 0.5) * size, y1 + Math.sin(angle - 0.5) * size); radarCtx.lineTo(x1 + Math.cos(angle + 0.5) * size, y1 + Math.sin(angle + 0.5) * size); radarCtx.closePath(); radarCtx.fill(); }
  function drawHandle(x, y) { radarCtx.beginPath(); radarCtx.fillStyle = "#fff"; radarCtx.arc(x, y, 7, 0, Math.PI * 2); radarCtx.fill(); radarCtx.stroke(); }
  function drawMotion(o, cx, cy, scale) {
    if (!state.editing || o.motion === "stationary") return;
    const active = o.id === state.selected, color = active ? "#b82424" : "#2d6cdf";
    radarCtx.save(); radarCtx.strokeStyle = color; radarCtx.fillStyle = color; radarCtx.lineWidth = active ? 2 : 1.5;
    if (o.motion === "orbit") { const x = cx, y = cy; radarCtx.beginPath(); radarCtx.arc(x, y, Math.max(8, o.radius * scale), 0, Math.PI * 2); radarCtx.stroke(); if (active) { const sx = x + Math.cos(o.phase || 0) * o.radius * scale, sy = y - Math.sin(o.phase || 0) * o.radius * scale; drawHandle(sx, sy); drawArrow(x, y, sx, sy); } }
    else if (o.motion === "approach") { const x = cx + (o.x - state.listener.x) * scale, y = cy - (o.z - state.listener.z) * scale; radarCtx.beginPath(); radarCtx.moveTo(x, y); radarCtx.lineTo(cx, cy); radarCtx.stroke(); if (active) { drawArrow(x, y, cx, cy); drawHandle(x, y); } }
    else { const x = cx + (o.centerX - state.listener.x) * scale, y = cy - (o.centerZ - state.listener.z) * scale; const horizontal = o.path !== "vertical"; const ax = horizontal ? o.radius * scale : 0, ay = horizontal ? 0 : o.radius * scale; const startX = x - ax, startY = y + ay, endX = x + ax, endY = y - ay; radarCtx.beginPath(); radarCtx.moveTo(startX, startY); radarCtx.lineTo(endX, endY); radarCtx.stroke(); drawArrow(startX, startY, endX, endY); }
    radarCtx.restore();
  }
  function drawRadarShape(o, x, y, density) { const size = (state.editing ? 24 : 20) * density, color = o.id === state.selected && state.editing ? "#b82424" : "#111"; radarCtx.save(); radarCtx.translate(x, y); radarCtx.fillStyle = color; if (o.shape === "circle") { radarCtx.beginPath(); radarCtx.arc(0, 0, size / 2, 0, Math.PI * 2); radarCtx.fill(); } else { radarCtx.fillRect(-size / 2, -size / 2, size, size); if (o.shape === "person" || o.shape === "car") { radarCtx.fillStyle = "#fff"; radarCtx.font = `${Math.round(size * 0.65)}px monospace`; radarCtx.textAlign = "center"; radarCtx.textBaseline = "middle"; radarCtx.fillText(o.shape === "person" ? "P" : "C", 0, density); } } radarCtx.restore(); }
  function drawRadar() {
    const w = radar.width, h = radar.height, cx = w / 2, cy = h / 2, scale = w / (RADAR_RANGE * 2), density = w / Math.max(1, radar.getBoundingClientRect().width);
    ...
    state.obstacles.forEach((o) => { drawMotion(o, cx, cy, scale); const dx = o.x - state.listener.x, dz = o.z - state.listener.z, x = cx + dx * scale, y = cy - dz * scale; if (x < -40 || x > w + 40 || y < -40 || y > h + 40) return; drawRadarShape(o, x, y, density); radarCtx.fillStyle = "#111"; radarCtx.textAlign = "left"; radarCtx.textBaseline = "alphabetic"; radarCtx.font = `${(state.editing ? 14 : 10) * density}px monospace`; radarCtx.fillText(o.label, x + 15 * density, y - 9 * density); });
    radarCtx.strokeStyle = "#111"; radarCtx.lineWidth = 2 * density; radarCtx.beginPath(); radarCtx.arc(cx, cy, (state.editing ? 10 : 8) * density, 0, Math.PI * 2); radarCtx.stroke(); const f = forward(), headingLength = (state.editing ? 30 : 22) * density; radarCtx.beginPath(); radarCtx.moveTo(cx, cy); radarCtx.lineTo(cx + f.x * headingLength, cy - f.z * headingLength); radarCtx.stroke();
  }
  ...
  const audio = { ctx: null, master: null, voices: new Map(), sweepStart: 0, sweepDuration: 1.05, pauseDuration: 0.25, nextSweepAt: 0, active: false, renderTarget: null };
  ...
  function startSweep() { if (!audio.ctx || !audio.active || state.mode !== "baseline" || audio.baseline) return; const source = audio.ctx.createBufferSource(); source.buffer = makeVoiBuffer(captureVoi()); source.connect(audio.master); audio.sweepStart = audio.ctx.currentTime; audio.nextSweepAt = Infinity; audio.baseline = source; source.start(); source.onended = () => { if (audio.baseline === source) { audio.baseline = null; audio.nextSweepAt = audio.ctx.currentTime + audio.pauseDuration; } }; }
  function startAudio() { leaveEditMode(); createAudio(); audio.ctx.resume(); audio.active = true; $("startAudio").dataset.audioState = "playing"; $("startAudio").textContent = "Stop Audio"; syncEditUi(); syncAudioPositions(); startSweep(); log("audio started"); }
  function stopAudio() { if (audio.ctx) { if (audio.baseline) { try { audio.baseline.stop(); } catch {} audio.baseline = null; } audio.nextSweepAt = 0; audio.ctx.suspend(); } audio.active = false; syncAudioPositions(); $("startAudio").dataset.audioState = "stopped"; $("startAudio").textContent = "Start Audio"; enterEditMode(); }
  ...
  function updateSweep() { if (!audio.active || state.mode !== "baseline" || !audio.ctx || !audio.baseline) { sweepLine.style.display = "none"; return; } sweepLine.style.display = "block"; const phase = Math.max(0, Math.min(1, (audio.ctx.currentTime - audio.sweepStart) / audio.sweepDuration)); sweepLine.style.left = `${phase * 100}%`; }
  ...
  function renderList() {
    ...
    state.obstacles.forEach((o) => {
      const row = document.createElement("div"); row.className = `obstacle-row${o.id === state.selected ? " selected" : ""}`; row.dataset.obstacleId = o.id;
      const label = document.createElement("span"); label.className = "row-label"; label.textContent = o.label || o.id;
      const shape = document.createElement("span"); shape.className = "shape-name"; shape.textContent = o.shape.charAt(0).toUpperCase() + o.shape.slice(1);
      const motion = document.createElement("span"); motion.className = "row-state"; motion.textContent = o.motion;
      const remove = document.createElement("button"); remove.className = "row-remove"; remove.type = "button"; remove.textContent = "×"; remove.title = `Remove ${o.label || o.id}`; remove.setAttribute("aria-label", `Remove ${o.label || o.id}`);
      remove.onclick = (event) => { event.stopPropagation(); removeObstacleById(o.id); };
      row.append(label, shape, motion, remove); row.onclick = () => { state.selected = o.id; renderList(); renderEditor(); }; list.appendChild(row);
    });
  }
  function syncEditUi() { worldCard.classList.toggle("editing", state.editing); document.body.classList.toggle("editing-mode", state.editing); document.body.classList.toggle("playback-mode", audio.active); $("editPalette").hidden = !state.editing; $("motionPanel").hidden = !state.editing || !state.obstacles.some((o) => o.id === state.selected); const o = state.obstacles.find((x) => x.id === state.selected); if (o) { $("motionObjectLabel").textContent = `Obstacle ${o.label}`; $("motionPanel").querySelectorAll("[data-motion-tool]").forEach((button) => button.setAttribute("aria-pressed", (button.dataset.motionTool === (o.motion === "patrol" ? o.path : o.motion)) ? "true" : "false")); $("motionHint").textContent = o.motion === "stationary" ? "Drag this obstacle to place it." : o.motion === "patrol" ? "Drag either red endpoint to set coverage." : o.motion === "orbit" ? "Drag the red handle to set radius." : "Drag the red path or obstacle to adjust it."; } }
  ...
  function radarMotionHandleAt(event) { const o = state.obstacles.find((x) => x.id === state.selected); if (!o || !state.editing || o.motion === "stationary") return null; const p = radarWorldPoint(event), tolerance = Math.max(0.35, RADAR_RANGE / 28); if (o.motion === "orbit") { if (Math.hypot(o.x - p.x, o.z - p.z) <= tolerance) return { o, kind: "orbit" }; return null; } if (o.motion === "patrol") { const horizontal = o.path !== "vertical", a = horizontal ? { x: o.centerX - o.radius, z: o.centerZ } : { x: o.centerX, z: o.centerZ - o.radius }, b = horizontal ? { x: o.centerX + o.radius, z: o.centerZ } : { x: o.centerX, z: o.centerZ + o.radius }; if (Math.hypot(a.x - p.x, a.z - p.z) <= tolerance) return { o, kind: "patrol", sign: -1 }; if (Math.hypot(b.x - p.x, b.z - p.z) <= tolerance) return { o, kind: "patrol", sign: 1 }; } return null; }
  function setMotionTool(tool) { const o = state.obstacles.find((x) => x.id === state.selected); if (!o) return; if (tool === "stationary") { o.motion = "stationary"; } else if (tool === "horizontal" || tool === "vertical") { o.motion = "patrol"; o.path = tool; o.centerX = o.x; o.centerZ = o.z; o.radius = Math.max(1, o.radius || 2); } else if (tool === "orbit") { o.motion = "orbit"; o.centerX = state.listener.x; o.centerZ = state.listener.z; o.radius = Math.max(0.5, Math.hypot(o.x - state.listener.x, o.z - state.listener.z)); o.phase = Math.atan2(o.z - state.listener.z, o.x - state.listener.x); } else if (tool === "approach") { o.motion = "approach"; o.centerX = o.x; o.centerZ = o.z; o.radius = 1; } syncEditUi(); syncMeshes(); renderList(); renderEditor(); drawRadar(); }
  function nextObstacleId() { for (let i = 0; i < 26; i++) { const id = String.fromCharCode(65 + i); if (!state.obstacles.some((o) => o.id === id)) return id; } return null; }
  function addObstacleAt(shape, point) { const id = nextObstacleId(); if (!id) return; const base = structuredClone(defaults.obstacles[0]); const o = { ...base, id, label: id, shape, x: point.x, z: point.z, motion: "stationary" }; if (shape === "person") Object.assign(o, { width: 0.7, depth: 0.5, height: 1.8 }); if (shape === "car") Object.assign(o, { width: 1.8, depth: 4, height: 1.4 }); state.obstacles.push(o); state.selected = id; syncMeshes(); renderList(); renderEditor(); syncEditUi(); }
  function bindRadarEditing() { const overlay = document.querySelector(".radar-overlay"); const open = () => { if (!audio.active) enterEditMode(); }; overlay.addEventListener("click", open); overlay.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); open(); } }); radar.addEventListener("pointerdown", (e) => { if (!state.editing) return; const handle = radarMotionHandleAt(e), o = radarObstacleAt(e); if (handle) { state.selected = handle.o.id; radarDrag = { type: "trajectory", ...handle }; } else if (o) { state.selected = o.id; radarDrag = { type: "obstacle", id: o.id }; } else { radar.style.cursor = "crosshair"; return; } radar.style.cursor = "grabbing"; renderList(); renderEditor(); syncEditUi(); radar.setPointerCapture(e.pointerId); }); radar.addEventListener("pointermove", (e) => { if (!state.editing) return; if (!radarDrag) { radar.style.cursor = radarMotionHandleAt(e) || radarObstacleAt(e) ? "grab" : "crosshair"; return; } const o = state.obstacles.find((x) => x.id === radarDrag.id || x.id === radarDrag.o?.id); if (!o) return; const p = radarWorldPoint(e); if (radarDrag.type === "trajectory" && o.motion === "patrol") { if (o.path === "vertical") o.radius = Math.max(0.35, Math.abs(p.z - o.centerZ)); else o.radius = Math.max(0.35, Math.abs(p.x - o.centerX)); } else { o.x = p.x; o.z = p.z; if (o.motion === "orbit") { o.radius = Math.max(0.5, Math.hypot(o.x - state.listener.x, o.z - state.listener.z)); o.phase = Math.atan2(o.z - state.listener.z, o.x - state.listener.x); } else if (o.motion === "patrol") { o.centerX = o.x; o.centerZ = o.z; } } syncMeshes(); renderEditor(); drawRadar(); }); window.addEventListener("pointerup", () => { radarDrag = null; radar.style.cursor = state.editing ? "crosshair" : ""; }); radar.addEventListener("dragover", (e) => { if (state.editing) e.preventDefault(); }); radar.addEventListener("drop", (e) => { if (!state.editing) return; e.preventDefault(); const shape = e.dataTransfer.getData("text/plain"); if (shape) addObstacleAt(shape, radarWorldPoint(e)); }); document.querySelectorAll(".shape-tool").forEach((tool) => { tool.addEventListener("dragstart", (e) => e.dataTransfer.setData("text/plain", tool.dataset.shape)); tool.addEventListener("click", () => { if (!state.editing) return; addObstacleAt(tool.dataset.shape, { x: state.listener.x, z: state.listener.z + 3 }); }); }); document.querySelectorAll("[data-motion-tool]").forEach((button) => button.addEventListener("click", () => setMotionTool(button.dataset.motionTool))); $("closeMotionPanel").onclick = () => { state.selected = null; syncEditUi(); renderList(); drawRadar(); }; }
  ...
  function resetScene() { Object.assign(state, { mode: "baseline", paused: false, editing: !audio.active, selected: "A", voiceLow: defaults.voiceLow, voiceHigh: defaults.voiceHigh, hrtfLow: defaults.hrtfLow, hrtfHigh: defaults.hrtfHigh, listener: { ...defaults.listener }, obstacles: structuredClone(defaults.obstacles) }); $("modeBaseline").setAttribute("aria-pressed", "true"); $("modeHrtf").setAttribute("aria-pressed", "false"); $("pauseMotion").textContent = "Pause Motion"; syncMeshes(); updateCameras(); renderList(); renderEditor(); setRanges(); syncEditUi(); log("scene reset"); }
  function setRanges() { ["voiceLow", "voiceHigh", "hrtfLow", "hrtfHigh"].forEach((id) => { $(id).value = state[id]; }); }
  function addObstacle(copy) { const id = nextObstacleId(); if (!id) return; const index = state.obstacles.length; const o = { ...(copy || defaults.obstacles[0]), id, label: id, x: index * 1.5 - 1.5, z: 4, shape: copy?.shape || "cube" }; state.obstacles.push(o); state.selected = o.id; syncMeshes(); renderList(); renderEditor(); syncEditUi(); }
  function removeObstacleById(id) { const index = state.obstacles.findIndex((o) => o.id === id); if (index < 0) return; state.obstacles.splice(index, 1); if (state.selected === id) state.selected = state.obstacles[Math.min(index, state.obstacles.length - 1)]?.id || null; syncMeshes(); syncAudioPositions(); renderList(); renderEditor(); syncEditUi(); drawRadar(); }
  ...
  function frame(now) { const t = (now - state.startedAt) / 1000; state.obstacles.forEach((o) => applyMotion(o, t)); syncMeshes(); updateCameras(); drawRadar(); syncAudioPositions(); if (audio.active && state.mode === "baseline" && !audio.baseline && audio.ctx.currentTime >= audio.nextSweepAt) startSweep(); if (state.mode === "hrtf" && audio.baseline) { try { audio.baseline.stop(); } catch {} audio.baseline = null; } updateSweep(); updateTelemetry(); renderer.render(scene, camera); requestAnimationFrame(frame); }
