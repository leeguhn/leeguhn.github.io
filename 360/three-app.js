(() => {
  ...
  function drawMotion(o, cx, cy, scale) {
    ...
    else { const x = cx + (o.centerX - state.listener.x) * scale, y = cy - (o.centerZ - state.listener.z) * scale; const horizontal = o.path !== "vertical"; const ax = horizontal ? o.radius * scale : 0, ay = horizontal ? 0 : o.radius * scale; radarCtx.beginPath(); radarCtx.moveTo(x - ax, y + ay); radarCtx.lineTo(x + ax, y - ay); radarCtx.stroke(); drawHandle(x - ax, y + ay); drawHandle(x + ax, y - ay); drawArrow(x - ax, y + ay, x + ax, y - ay); }
    radarCtx.restore();
  }
  function drawRadarShape(o, x, y, scale) { const width = Math.max(7, o.width * scale), depth = Math.max(7, o.depth * scale); radarCtx.save(); radarCtx.translate(x, y); radarCtx.rotate(-(o.heading || 0) * Math.PI / 180); radarCtx.fillStyle = o.id === state.selected && state.editing ? "#b82424" : "#111"; if (o.shape === "circle") { radarCtx.beginPath(); radarCtx.arc(0, 0, Math.max(width, depth) / 2, 0, Math.PI * 2); radarCtx.fill(); } else if (o.shape === "person") { radarCtx.fillRect(-width * 0.28, -depth * 0.15, width * 0.56, depth * 0.7); radarCtx.beginPath(); radarCtx.arc(0, -depth * 0.3, Math.max(3, width * 0.25), 0, Math.PI * 2); radarCtx.fill(); } else if (o.shape === "car") { radarCtx.beginPath(); radarCtx.roundRect(-width / 2, -depth / 2, width, depth, Math.min(width, depth) * 0.16); radarCtx.fill(); } else { radarCtx.fillRect(-width / 2, -depth / 2, width, depth); } radarCtx.restore(); }
  function drawRadar() {
    ...
    state.obstacles.forEach((o) => { drawMotion(o, cx, cy, scale); const dx = o.x - state.listener.x, dz = o.z - state.listener.z, x = cx + dx * scale, y = cy - dz * scale; if (x < -40 || x > w + 40 || y < -40 || y > h + 40) return; drawRadarShape(o, x, y, scale); radarCtx.fillStyle = "#111"; radarCtx.font = `${state.editing ? 14 : 10}px monospace`; radarCtx.fillText(o.label, x + 10, y - 9); });
  ...
  function radarMotionHandleAt(event) { const o = state.obstacles.find((x) => x.id === state.selected); if (!o || !state.editing || o.motion === "stationary") return null; const p = radarWorldPoint(event), tolerance = Math.max(0.35, RADAR_RANGE / 28); if (o.motion === "orbit") { if (Math.hypot(o.x - p.x, o.z - p.z) <= tolerance) return { o, type: "orbit" }; return null; } if (o.motion === "patrol") { const horizontal = o.path !== "vertical", a = horizontal ? { x: o.centerX - o.radius, z: o.centerZ } : { x: o.centerX, z: o.centerZ - o.radius }, b = horizontal ? { x: o.centerX + o.radius, z: o.centerZ } : { x: o.centerX, z: o.centerZ + o.radius }; if (Math.hypot(a.x - p.x, a.z - p.z) <= tolerance) return { o, type: "patrol", sign: -1 }; if (Math.hypot(b.x - p.x, b.z - p.z) <= tolerance) return { o, type: "patrol", sign: 1 }; } return null; }
