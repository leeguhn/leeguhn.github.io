(() => {
  ...
  const state = { mode: "baseline", paused: false, selected: "A", voiceLow: defaults.voiceLow, voiceHigh: defaults.voiceHigh, hrtfLow: defaults.hrtfLow, hrtfHigh: defaults.hrtfHigh, listener: { ...defaults.listener }, obstacles: structuredClone(defaults.obstacles), startedAt: performance.now() };
  const logs = [];
  ...
  function applyMotion(o, t) {
    if (state.paused || o.motion === "stationary") return;
    if (o.motion === "orbit") { const a = t * o.degrees * Math.PI / 180; o.x = o.centerX + Math.cos(a) * o.radius; o.z = o.centerZ + Math.sin(a) * o.radius; }
    if (o.motion === "patrol") { const p = (Math.sin(t * o.degrees * Math.PI / 180) + 1) / 2; o.x = o.centerX + (p * 2 - 1) * o.radius; }
  ...
  function drawRadar() {
    const w = radar.width, h = radar.height, cx = w / 2, cy = h / 2, scale = 18;
  ...
  function resetScene() { Object.assign(state, { mode: "baseline", paused: false, selected: "A", voiceLow: defaults.voiceLow, voiceHigh: defaults.voiceHigh, hrtfLow: defaults.hrtfLow, hrtfHigh: defaults.hrtfHigh, listener: { ...defaults.listener }, obstacles: structuredClone(defaults.obstacles) }); $("modeBaseline").setAttribute("aria-pressed", "true"); $("modeHrtf").setAttribute("aria-pressed", "false"); $("pauseMotion").textContent = "Pause Motion"; syncMeshes(); updateCameras(); renderList(); renderEditor(); setRanges(); log("scene reset"); }
  function setRanges() { ["voiceLow", "voiceHigh", "hrtfLow", "hrtfHigh"].forEach((id) => { $(id).value = state[id]; }); }
  function addObstacle(copy) { const index = state.obstacles.length; const o = { ...(copy || defaults.obstacles[0]), id: String.fromCharCode(65 + index), label: String.fromCharCode(65 + index), x: index * 1.5 - 1.5, z: 4, shape: copy?.shape || "cube" }; state.obstacles.push(o); state.selected = o.id; syncMeshes(); renderList(); renderEditor(); }
  ...
  resize(); window.addEventListener("resize", resize); syncMeshes(); updateCameras(); renderList(); renderEditor(); setRanges(); log("3D scene ready"); requestAnimationFrame(frame);
})();
