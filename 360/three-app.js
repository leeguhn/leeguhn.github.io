(() => {
  ...
  function drawMotion(o, cx, cy, scale) {
    ...
    if (o.motion === "orbit") { const x = cx, y = cy; radarCtx.beginPath(); radarCtx.arc(x, y, Math.max(8, o.radius * scale), 0, Math.PI * 2); radarCtx.stroke(); const sx = x + Math.cos(o.phase || 0) * o.radius * scale, sy = y - Math.sin(o.phase || 0) * o.radius * scale; drawHandle(sx, sy); drawArrow(x, y, sx, sy); }
    else if (o.motion === "approach") { const x = cx + (o.x - state.listener.x) * scale, y = cy - (o.z - state.listener.z) * scale; radarCtx.beginPath(); radarCtx.moveTo(x, y); radarCtx.lineTo(cx, cy); radarCtx.stroke(); drawArrow(x, y, cx, cy); drawHandle(x, y); }
