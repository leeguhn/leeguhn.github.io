(() => {
  ...
  function drawMotion(o, cx, cy, scale) {
    ...
    if (o.motion === "orbit") { const x = cx, y = cy; radarCtx.beginPath(); radarCtx.arc(x, y, Math.max(8, o.radius * scale), 0, Math.PI * 2); radarCtx.stroke(); const sx = x + Math.cos(o.phase || 0) * o.radius * scale, sy = y - Math.sin(o.phase || 0) * o.radius * scale; drawArrow(x, y, sx, sy); drawHandle(sx, sy); }
    ...
    else { const x = cx + (o.centerX - state.listener.x) * scale, y = cy - (o.centerZ - state.listener.z) * scale; const horizontal = o.path !== "vertical"; const ax = horizontal ? o.radius * scale : 0, ay = horizontal ? 0 : o.radius * scale; radarCtx.beginPath(); radarCtx.moveTo(x - ax, y + ay); radarCtx.lineTo(x + ax, y - ay); radarCtx.stroke(); drawArrow(x - ax, y + ay, x + ax, y - ay); drawArrow(x + ax, y - ay, x - ax, y + ay); drawHandle(x - ax, y + ay); drawHandle(x + ax, y - ay); }
    radarCtx.restore();
  }
  function drawRadar() {
