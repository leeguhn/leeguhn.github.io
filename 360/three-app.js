(() => {
  ...
  function drawRadarShape(o, x, y) { const size = state.editing ? 24 : 14, color = o.id === state.selected && state.editing ? "#b82424" : "#111"; radarCtx.save(); radarCtx.translate(x, y); radarCtx.fillStyle = color; if (o.shape === "circle") { radarCtx.beginPath(); radarCtx.arc(0, 0, size / 2, 0, Math.PI * 2); radarCtx.fill(); } else { radarCtx.fillRect(-size / 2, -size / 2, size, size); if (o.shape === "person" || o.shape === "car") { radarCtx.fillStyle = "#fff"; radarCtx.font = `${Math.round(size * 0.65)}px monospace`; radarCtx.textAlign = "center"; radarCtx.textBaseline = "middle"; radarCtx.fillText(o.shape === "person" ? "P" : "C", 0, 1); } } radarCtx.restore(); }
  function drawRadar() {
    ...
    state.obstacles.forEach((o) => { drawMotion(o, cx, cy, scale); const dx = o.x - state.listener.x, dz = o.z - state.listener.z, x = cx + dx * scale, y = cy - dz * scale; if (x < -40 || x > w + 40 || y < -40 || y > h + 40) return; drawRadarShape(o, x, y); radarCtx.fillStyle = "#111"; radarCtx.textAlign = "left"; radarCtx.textBaseline = "alphabetic"; radarCtx.font = `${state.editing ? 14 : 10}px monospace`; radarCtx.fillText(o.label, x + 15, y - 9); });
