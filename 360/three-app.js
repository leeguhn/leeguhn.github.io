(() => {
  ...
  function stopAudio() { if (audio.ctx) { if (audio.baseline) { try { audio.baseline.stop(); } catch {} audio.baseline = null; } audio.ctx.suspend(); } audio.active = false; syncAudioPositions(); $("startAudio").dataset.audioState = "stopped"; $("startAudio").textContent = "Start Audio"; enterEditMode(); }
